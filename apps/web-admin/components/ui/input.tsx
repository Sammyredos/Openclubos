...
function SearchableSelect({
  ...
  return (
    <div ref={ref} className={cn("relative", className)}>
      {trigger ? (
        <div onClick={() => !disabled && setOpen((v) => !v)} className={cn("w-full", disabled && "opacity-50 cursor-not-allowed pointer-events-none")}>
          {trigger}
        </div>
      ) : (
        <button
        ...
        >
          <div className="flex items-center gap-2.5 flex-1 overflow-hidden pr-2">
            {selected?.image ? (
              <img src={selected.image} className="w-6 h-6 rounded-lg object-contain shrink-0 border border-gray-200/90 bg-white p-0.5 shadow-2xs" alt="" />
            ) : selected?.icon ? (
              <div className="w-6 h-6 rounded-full bg-emerald-50 flex items-center justify-center shrink-0 border border-emerald-100">
                <selected.icon className="w-3.5 h-3.5 text-openclub-800" />
              </div>
            ) : null}
                ...
                >
                  <div className="flex items-center justify-between w-full gap-2 overflow-hidden">
                    <div className="flex items-center gap-2.5 flex-1 min-w-0">
                      {o.image ? (
                        <img src={o.image} className="w-7 h-7 rounded-lg object-contain shrink-0 border border-gray-200/90 bg-white p-0.5 shadow-2xs" alt="" />
                      ) : o.icon ? (
