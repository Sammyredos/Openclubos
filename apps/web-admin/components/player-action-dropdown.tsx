  ...
  return (
    <>
      <button
        ...
        className={
          variant === "hamburger"
            ? "bg-emerald-50 text-openclub-800 border border-emerald-100 text-sm font-bold rounded-md cursor-pointer focus:ring-0 opacity-0 group-hover/player:opacity-100 transition-opacity disabled:opacity-0 disabled:cursor-not-allowed hover:bg-emerald-100 flex items-center justify-center w-[24px] h-[24px]"
            : "bg-[#f5faf6] text-[#15803D] border border-[#e1efe5] text-[11px] font-medium rounded-md px-2 py-1.5 cursor-pointer focus:ring-0 hover:bg-[#e1efe5] transition-colors disabled:opacity-0 disabled:cursor-not-allowed shadow-sm flex items-center gap-1"
        }
      >
        {variant === "hamburger" ? "☰" : (
          <>
      ...
      >
        ...
        <div className="p-1 flex flex-col gap-0.5">
          ...
          {groupingsData?.groups?.length > 0 && (
            <>
              {group && <div className="h-px bg-gray-100 my-1 mx-2" />}
              {groupingsData.groups.map((g: any) => {
                ...
                return (
                  <button
                    ...
                    className="text-left px-2 py-1.5 text-gray-700 hover:bg-gray-100 rounded-md transition-colors flex items-center justify-between"
                  >
                    <span className="truncate pr-2">{group ? `Move Player to ${g.name}` : `Assign to ${g.name}`}</span>
                    {isFull ? (
