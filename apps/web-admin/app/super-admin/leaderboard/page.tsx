...
export default function LeaderboardDirectoryPage() {
  ...
  const uniqueClubs = useMemo(() => {
    const c = new Set(tournaments.map(t => t.club?.name || "Unknown Club"));
  ...
  const filteredTournaments: TournamentRow[] = useMemo(() => {
    ...
    return tournaments
      .filter((t) => {
        ...
        const clubName = t.club?.name || "Unknown Club";
        if (clubFilter !== "All Organizers" && clubName !== clubFilter) return false;
  ...
  }, [tournaments, debouncedSearchQuery, clubFilter, statusFilter, yearFilter, monthFilter]);

  const totalPages = Math.ceil(filteredTournaments.length / itemsPerPage);

  const openView = (t: TournamentRow) => {
    router.push(`/super-admin/leaderboard/${t.id}`);
  };
