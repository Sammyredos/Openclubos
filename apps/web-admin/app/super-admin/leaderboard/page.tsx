...
export default function TournamentsPage() {
  ...
  const rows = tournaments.map((t) => {
    ...
    // Format
    const rawFormat = typeof t.scoringType === "string" ? t.scoringType : (t.format || "STROKE_PLAY");
    const formatStr = rawFormat.replace(/_/g, " ").toLowerCase().replace(/\b\w/g, (l: string) => l.toUpperCase());

    // Rounds
    const holes = t.holes || 18;
    const daysSpan = t.startDate && t.endDate ? Math.round((new Date(t.endDate).getTime() - new Date(t.startDate).getTime()) / (1000 * 60 * 60 * 24)) + 1 : 1;
    const totalRounds = Math.max(daysSpan, Math.max(1, Math.round(holes / 18)));
    const currentRound = t.status === "COMPLETED" ? totalRounds : (t.status === "ONGOING" ? 1 : 0);
    const roundsStr = `${currentRound} / ${totalRounds}`;

    // Last Updated
    const lastUpdatedDate = t.updatedAt ? new Date(t.updatedAt) : new Date(t.createdAt);
    const diffHours = Math.floor((new Date().getTime() - lastUpdatedDate.getTime()) / (1000 * 60 * 60));
    let lastUpdatedStr = "Just now";
    if (diffHours > 24) {
      lastUpdatedStr = `${Math.floor(diffHours / 24)}d Ago`;
    } else if (diffHours > 0) {
      lastUpdatedStr = `${diffHours}h Ago`;
    }
  ...
  const openView = (tournament: TournamentRow) => {
    closeDropdown();
    router.push(`/super-admin/leaderboard/${tournament.id}`);
  };
                  ...
                  {error ? (
                    ...
                    filteredTournaments.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((t) => (
                      <tr key={t.id} className="hover:bg-background/50 transition-colors group">
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-3 min-w-[220px]">
                            ...
                            <div className="flex flex-col min-w-0 gap-0.5">
                              ...
                              <span className="text-gray-500 text-[12px] font-normal truncate mt-0.5" title={t.subtitle}>{t.subtitle}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                        ...
                        <td className="px-6 py-5">
                          <div className="flex flex-col items-start gap-0.5">
                            <span className="text-gray-500 text-[13px] font-medium leading-tight">{t.lastUpdated}</span>
                          </div>
                        </td>
                        <td className="px-6 py-5">
                          <div className="flex items-center justify-center gap-2">
